...
import { Box, Button, Input, List, ListItem, Text, VStack, HStack, Flex, useBreakpointValue, Checkbox, Link, Select, Modal, ModalOverlay, ModalContent, ModalHeader, ModalFooter, ModalBody, ModalCloseButton } from '@chakra-ui/react';
...
useEffect(() => {
    ...
    if (speed <= spinThreshold && isSpinning && !isChecked) {
      setIsSpinning(false);
  ...
  return (  
    ...
    <Flex alignItems="center" justifyContent="flex-start"  flexDirection="row" transform="translateX(-45%)">
      ...
      <HStack 
        flex={1}
        p={1}
        spacing={0} // Set spacing to 0 to remove space between items
        align="stretch"
        width={{ base: '55%', md: '45%' }}
      ...
      <Text color="gray.700" fontWeight="bold" letterSpacing="wide" >
        {isSpinning ? "Spin in progress... Scores evaluated when object is stationary." : "Spin complete."}
    </Text>
          <Text>
